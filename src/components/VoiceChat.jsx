...
const VoiceChat = ({ onSendMessage, isProcessing, messages, onClearChat }) => {
  ...
  useEffect(() => {
    ...
    if (apiKey) {
      ...
      console.log('🔊 ElevenLabs TTS:', success ? 'Initialized ✅' : 'Failed to initialize ❌');
    } else {
      console.warn('⚠️ VITE_ELEVENLABS_API_KEY not found - Voice responses disabled');
  ...
  const speakText = async (text) => {
    ...
    try {
      ...
      console.log('🗣️ Generating speech with ElevenLabs:', { language: currentLang, text: text.substring(0, 50) + '...' });
        ...
        <AnimatePresence mode="popLayout">
          {messages.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-xl font-semibold mb-2">{t('planoraAi.voiceChat.startConversation')}</h3>
              <p className="text-sm">{t('planoraAi.voiceChat.tapMic')}</p>
            </motion.div>
          ) : (
                ...
                >
                  ...
                  {message.isRestricted && (
                    <p className="text-xs mt-2 opacity-75 italic">
                      (This query is outside Planora's scope)
                    </p>
                  )}
                </div>
              </motion.div>
          ...
          >
            <div className="bg-card border border-border rounded-2xl px-5 py-3">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-sm">{t('planoraAi.voiceChat.thinking')}</span>
              </div>
            </div>
          </motion.div>
        )}
            ...
            <div className="relative bg-violet/20 backdrop-blur border-2 border-violet rounded-2xl px-6 py-3 shadow-lg shadow-violet/30">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-violet">
                  {currentLang === 'ar' ? 'جاري تحضير الصوت...' : 'Generating voice...'}
                </span>
              </div>
            </div>
          </motion.div>
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-amber">
                    {currentLang === 'ar' ? 'بلانورا تتحدث...' : 'Planora is speaking...'}
                  </span>
                </div>
          ...
          <>
            <div className="flex items-center justify-center gap-4">
              ...
              <motion.button
                ...
                title={speechEnabled ? t('planoraAi.voiceChat.disableVoice') : t('planoraAi.voiceChat.enableVoice')}
              >
                {speechEnabled ? (
                  isSpeaking ? (
                    <motion.div
            ...
            <div className="text-center mt-4">
              {!isSpeechSupported ? (
                ...
                <p className="text-sm text-muted-foreground">{t('planoraAi.voiceChat.processing')}</p>
              ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  placeholder={currentLang === 'ar' ? 'اكتب رسالتك هنا...' : 'Type your message...'}
                  disabled={isProcessing}
                  className={`w-full px-4 py-3 rounded-xl border-2 border-border bg-background focus:border-amber focus:outline-none transition-all disabled:opacity-50 ${
              ...
              <motion.button
                ...
                className="px-4 py-3 rounded-xl bg-gradient-amber text-violet-dark shadow-lg shadow-amber/30 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {isProcessing ? (
          ...
          >
            {inputMode === 'voice' ? (
              <>
                ...
                <span>{currentLang === 'ar' ? 'التبديل إلى الكتابة' : 'Switch to Text'}</span>
              </>
            ) : (
              <>
                <Mic className="w-4 h-4" />
                <span>{currentLang === 'ar' ? 'التبديل إلى الصوت' : 'Switch to Voice'}</span>
              </>
            )}
          </motion.button>
        </div>
