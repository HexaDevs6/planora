...
export async function createOrGetConversation(currentUserId, otherUserId) {
  ...
  try {
    const { data: existing, error: rpcErr } = await supabase
      .rpc("find_existing_conversation", { usera: currentUserId, userb: otherUserId });

    if (rpcErr) {
      // Non-fatal: still continue to create fallback
      console.warn("find_existing_conversation rpc error:", rpcErr.message);
    }

    if (existing && existing.length && existing[0].find_existing_conversation) {
      // RPC returns row like [{ find_existing_conversation: <uuid> }]
      return existing[0].find_existing_conversation || existing[0].conversation_id || existing;
    }
    ...
    if (partErr) {
      console.warn("participants insert error (might already exist):", partErr.message);
      // continue anyway
    }
...
export function subscribeToNewMessages(conversationId, onNewMessage) {
  // subscribe to INSERTs on messages and filter by conversation_id
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        onNewMessage && onNewMessage(payload.new || payload.record || payload);
      }
    )
    .subscribe();
  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        onNewMessageForInbox && onNewMessageForInbox(payload.new || payload.record || payload);
      }
    )
    .subscribe();
